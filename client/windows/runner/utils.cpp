...
std::wstring Utf16FromUtf8(const std::string& utf8_string) {
  int length = ::MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS,
                                     ...
                                     nullptr, 0);
  std::wstring utf16_string(length, L'\0');
  ::MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, utf8_string.data(),
                        static_cast<int>(utf8_string.size()),
                        utf16_string.data(), length);
  return utf16_string;
}

std::wstring StateDir() {
  PWSTR local = nullptr;
  HRESULT found = ::SHGetKnownFolderPath(FOLDERID_LocalAppData,
                                         KF_FLAG_DEFAULT, nullptr, &local);
  std::wstring dir =
      SUCCEEDED(found) ? std::wstring(local) + L"\\Lumeo\\State" : L"";
  ::CoTaskMemFree(local);
  if (dir.empty()) {
    return dir;
  }
